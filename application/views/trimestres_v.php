			...
			<table id="my-table" class="responsive contentHorario">
				...
				<?php 
					...
					<tr> 
						...
						<td class="estado"><a href="#" onclick="cambiaEdoTrim(<?=$value['estado']?>, <?= $value['idtrim']?>)"><?php if ($value['estado'] == 1){echo "Activo";}else{echo "Inactivo";} ?></a></td>
						<td><a href="<?= base_url() ?>index.php/inicio_admin_c/horarioxTrimestre/<?= $value['idtrim']?>" target="_blank">Ver</a></td>
						<td><a href="#" onclick="editarTrimestre(<?= $value['idtrim']?>)">Editar</a></td>
